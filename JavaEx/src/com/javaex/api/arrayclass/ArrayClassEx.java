...
//		sortCustomEx();
	...
	private static void searchEx() {
		...
		System.out.println("홍길도의 인덱스:" + index);
	}
	...
	private static void copyArrayEx() {
		...
		//	방법 2. System 객체 이용복사
		System.arraycopy(arr,  0,  target, 0, 0);
					//(원본 배열, 복사 시작 위치(원본배열), 목적지, 복사 시작 위치(목적지에서), 복사할 길이)
		...
		target = Arrays.copyOfRange(arr, 5, arr.length);
								//(시작 인덱스, 끝 인덱스)
